...
class LambdaExpr extends Expr {
  constructor(param, body) {
    ...
    if(!(body instanceof Expr)) {
      throw new Error("body must be an Expr.")
    }
    this.param = param
...
function tryEvalRules(primitives, fns, rules, args) {
  ...
  throw new Error(`No rule matched ${JSON.stringify(args)}`)
}


function evalFunctionImpl(primitives, fns, f, args) {
  if(f.numParams != args.length) {
    throw new Error(`Function ${f.name} expects ${f.numParams} args, got ${args.length}`)
  }
  return tryEvalRules(primitives, fns, f.rules, args)
}
...
function evalFunction(primitives, fns, fName, args) {
  ...
  return evalFunctionImpl(primitives, fns, f, args)
}
