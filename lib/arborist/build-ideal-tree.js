...
const { ROSETTE_FUNCTIONS } = require('./rosette-functions.js')
...
const _parseVersionString = Symbol('parseVersionString')
const _tryParseWildcardBug = Symbol('tryParseWildcardBug')
const _tryParseWildcardMinor = Symbol('tryParseWildcardMinor')
const _rosetteConstraintFromSpec = Symbol('rosetteConstraintFromSpec')
const _rosetteConstraintFromDescriptor = Symbol('rosetteConstraintFromDescriptor')
const _reachableNodesStep = Symbol('reachableNodesStep')
const _versionStringSatSpec = Symbol('versionStringSatSpec')
...
module.exports = cls => class IdealTreeBuilder extends cls {
  ...
  async [_solveTreeWithRosette] () {
    const query = await this[_buildRosetteQuery]()
    const rosetteAnswer = await this[_performRosetteQuery](query)
    this[_decodeRosetteAnswer](rosetteAnswer)
  }
  ...
  async [_buildRosetteQuery] () {
    ...
    await this[_buildReachableRegistry](rootNode)
    ...
    const rosetteContextNode = contextNode.map(d => {
      const spec = d.constraintNpmSpec
      return {'packageToDependOn': d.packageToDependOn, 'constraint': this[_rosetteConstraintFromSpec](spec)}
    })

    const rosetteRegistry = Object.entries(reachableRegistry).map(name_versions_obj => {
      const name = name_versions_obj[0]
      const versions_obj = name_versions_obj[1]
      const versions_data = Object.entries(versions_obj).flatMap(version_deps_node => {
        const versionString = version_deps_node[0]
        const deps = version_deps_node[1].dependencies
        
        let version = null
        try {
          version = this[_parseVersionString](versionString)
        } catch {
          // If we can't parse the version, we don't include it in the query.
          return []
        }
        ...
        return [{'version': version, 'dependencies': rosetteDeps}]
      })
      return {'package': name, 'versions': versions_data}
    })
  ...
  [_parseVersionString] (versionString) {
    // TODO: Handle different types of versions, such as:
    // - Pure semver: x.y.z
    // - Semver + tag: x.y.z-alpha.1
    // - Arbitrary tag: asdf
    // - ???

    // For the versionDeserialize function, we can encode each type of version
    // as a vector, with the first element tagging the type of the version,
    // and other data following that.

    // But for now we'll just throw errors for non pure semver versions.

    const components = versionString.split('.')
    if(components.length == 3) {
      // TODO: We need to check that the components are numbers,
      // otherwise we send NaNs to Rosette
      const major = Number.parseInt(components[0])
      const minor = Number.parseInt(components[1])
      const bug = Number.parseInt(components[2])
      if(Number.isNaN(major) || Number.isNaN(minor) || Number.isNaN(bug)) {
        throw new Error(`Cannot parse version string ${versionString} (reason: NaN components)`)
      }
      return {'major': major, 'minor': minor, 'bug': bug}
    } else {
      throw new Error(`Cannot parse version string ${versionString} (reason: ${components.length} components rather than 3)`)
    }
  }

  [_tryParseWildcardBug] (specStr) {
    if(specStr == null) {
      return null
    }

    const components = specStr.split('.')
    if(components.length == 3) {
      const major = Number.parseInt(components[0])
      const minor = Number.parseInt(components[1])
      const bug = components[2]
      if(Number.isNaN(major) || Number.isNaN(minor)) {
        return null
      }
      if(bug == '*' || bug == 'x' || bug == 'X') {
        return {'wildcardBug': {'major': major, 'minor': minor}}
      }
      return null
  ...
  [_tryParseWildcardMinor] (specStr) {
    if(specStr == null) {
      return null
    }
    ...
    if(components.length == 3) {
      ...
      if((minor == '*' || minor == 'x' || minor == 'X') && (bug == '*' || bug == 'x' || bug == 'X')) {
        return {'wildcardMinor': {'major': major}}
      }
      return null
    } else if(components.length == 2) {
      const major = Number.parseInt(components[0])
  ...
  [_rosetteConstraintFromDescriptor] (desc) {
    const maybeWildcardBug = this[_tryParseWildcardBug](desc)
    const maybeWildcardMinor = this[_tryParseWildcardMinor](desc)

    if(desc.startsWith('^')) {
      try {
        return {'caret': this[_parseVersionString](desc.substring(1))}
      } catch {
        // If the version string is invalid, we'll just send an unsolvable constraint to Rosette
        return {'unsolveable': null}
      }
    } else if(desc.startsWith('~')) {
      try {
        return {'tilde': this[_parseVersionString](desc.substring(1))}
      } catch {
        // If the version string is invalid, we'll just send an unsolvable constraint to Rosette
        return {'unsolveable': null}
      }
    } else if(desc.startsWith('>=')) {
      const versionPart = desc.substring(2).trim()
  
      try {
        return {'ge': this[_parseVersionString](versionPart)}
      } catch {
        // If the version string is invalid, we'll just send an unsolvable constraint to Rosette
        return {'unsolveable': null}
      }
    } else if(maybeWildcardBug) {
      return maybeWildcardBug
    } else if(maybeWildcardMinor) {
      return maybeWildcardMinor
    } else {
      console.log('Unimplemented spec descriptor type: ' + desc)
      return {'unsolveable': null}
      // throw new Error('Unimplemented spec type: ' + spec.rawSpec)
    }
  }

  [_rosetteConstraintFromSpec] (spec) {
    
    if(spec.fetchSpec == 'latest' || spec.fetchSpec == '*') {
      return {'wildcardMajor': null}
    } else if(spec.type == 'version') {
      try {
        return {'exactly': this[_parseVersionString](spec.fetchSpec)}
      } catch (error) {
        console.error('Unimplemented spec type: ' + spec.rawSpec)
        console.error(error)
        // If the version string is invalid, we'll just send an unsolvable constraint to Rosette
        return {'unsolveable': null}
      }
      ...
      try {
        return this[_rosetteConstraintFromDescriptor](spec.fetchSpec)
      } catch(error) {
        console.error('Unimplemented spec type: ' + spec.rawSpec)
        console.error(error)
        return {'unsolveable': null}
      ...
      console.log('Unimplemented spec type: ' + spec.rawSpec)
      return {'unsolveable': null}
      // throw new Error('Unimplemented spec type: ' + spec.rawSpec)
  ...
  [_versionStringSatSpec] (versionString, spec) {
    return true

    // TODO: Enable this
    const constraint = this[_rosetteConstraintFromSpec](spec)
    let version = null
    try {
      version = this[_parseVersionString](versionString)
    } catch (error) {
      console.error(`Error parsing version: ${versionString}`)
      return false
    }
    return dsl.evalFunction(CONSTRAINT_PRIMITIVES, ROSETTE_FUNCTIONS, "constraintInterpretation", [constraint])(version)
  }

  async [_buildReachableRegistry] (rootNode) {
    ...
    while(this[_reachableNodesQueue].length > 0) {
      ...
      await this[_reachableNodesStep](context, node)
      context = false
  ...
  async [_reachableNodesStep] (isContextNode, node) {
    // if(node.name in this[_seenNodes]) {
    //   if(node.version in this[_seenNodes][node.name]) {
    //     console.log("Already seen: " + node.name + "@" + node.version)
    //     return
    ...
    // console.log(`Reachable node: ${node.name}@${node.version}`)

    const dependencies = Array.from(node.edgesOut)
      .filter(e => e[1].type == 'prod')
      .flatMap(e => {
        try {
          const spec = npa.resolve(e[1].name, e[1].spec, e[1].from.path)
          return [{'packageToDependOn': e[1].name, 'constraintNpmSpec': spec}]
        } catch (error) {
          console.error(error)
          return []
        }
    ...
    if(isContextNode) {
      ...
      if(!(node.version in this[_reachableRegistry][node.name])) {
        this[_reachableRegistry][node.name][node.version] = {"dependencies": dependencies, "node": node}
      }
    }
      ...
      return this[_fetchPackuments](spec).then(pack => {
        ...
        Object.entries(pack.versions).map(vers_pkg => {
          ...
          if(this[_markNodeSeen](newNode)) {
            if(this[_versionStringSatSpec](vers, spec)) {
    ...
    // return this[_reachableNodesStep]()
  }
