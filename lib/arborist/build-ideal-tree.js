...
const { rosetteConstraintFromRange, parseVersionString, buildPrereleaseOrder } = require('./rosette-translation-helpers.js')
    ...
    const rosetteRegistry = Object.entries(reachableRegistry).map(([name, versions_obj]) => {
      ...
      const versions_data = Object.entries(versions_obj).flatMap(([versionString, data]) => {
        ...
        const rosetteDeps = deps.map(d => {
          const constraint = rosetteConstraintFromRange(d.semverRange, prereleaseOrder)
