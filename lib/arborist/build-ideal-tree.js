...
module.exports = cls => class IdealTreeBuilder extends cls {
  ...
  async [_buildRosetteQuery] () {
    ...
    const rosetteRegistry = Object.entries(reachableRegistry).map(name_versions_obj => {
      ...
      const versions_data = Object.entries(versions_obj).map(version_deps_node => {
        const versionString = version_deps_node[0]
        const deps = version_deps_node[1].dependencies
        
        const version = this[_parseVersionString](versionString)
  ...
  [_parseVersionString] (versionString) {
    ...
    if(components.length == 3) {
      ...
      throw new Error('Cannot parse version string: ' + versionString)
    }
  }
  ...
  async [_reachableNodesStep] (isContextNode=false) {
    ...
    if(alreadySeen) {
      console.log("Already saw: " + node.name)
      return
    }
  ...
  async [_performRosetteQuery] (query) {
    ...
    await fs.writeFile(queryPath, JSON.stringify(query))
  ...
  [_decodeRosetteAnswer] (answer) {
    ...
    const nodes = g.vertices.map(v => {
      if(v.type == "RootContextVertex") {
    ...
    // TODO: This doesn't work for cyclic graphs (I think)
    // There are examples of elsewhere in this file that handle cycles by building links,
    // which I think is what I need to do here.
    for(let sourceIdx = 0; sourceIdx < out_edges.length; sourceIdx++) {
      let outs = out_edges[sourceIdx]
      for(let i = 0; i < outs.length; i++) {
        const destIdx = outs[i]
        nodes[destIdx].parent = nodes[sourceIdx]
      }
    }
  ...
  [_parseSettings] (options) {
    ...
    this[_useRosetteSolver] = !!options.useRosetteSolver
    if(this[_useRosetteSolver]) {
