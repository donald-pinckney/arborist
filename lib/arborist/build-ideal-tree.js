...
module.exports = cls => class IdealTreeBuilder extends cls {
  ...
  async [_buildRosetteQuery] () {
    ...
    const rosetteRegistry = Object.entries(reachableRegistry).map(name_versions_obj => {
      ...
      const versions_data = Object.entries(versions_obj).map(version_deps => {
        const versionString = version_deps[0]
        const deps = version_deps[1]
  ...
  async [_reachableNodesStep] (isContextNode=false) {
    ...
    if(isContextNode) {
      ...
      if(!(node.version in this[_reachableRegistry][node.name])) {
        this[_reachableRegistry][node.name][node.version] = dependencies
      } else {
        alreadySeen = true;
      }
    }
    ...
    await Promise.all(dependencies.map(dep => {
      ...
      return this[_fetchPackuments](spec).then(pack => {
        Object.entries(pack.versions).map(vers_pkg => {
          ...
          const newNode = this[_nodeFromPkg](pkg.name, pkg, node, legacyPeerDeps)
