...
const { ROSETTE_FUNCTIONS, CONSTRAINT_PRIMITIVES, sumCosts } = require('./rosette-functions.js')
  ...
  async [_buildRosetteQuery] () {
    ...
    const rosetteRegistry = names.map(name => {
      ...
      let versions_data = Object.entries(versions_obj).flatMap(([versionString, data]) => {
        ...
        return [{'version': jsonRosetteVersion, 'dependencies': rosetteDeps, 'cost_values': {'_unused': 0}}]
      })
    ...
    const optimization_objective_functions = [
      sumCosts((pg) => dsl.helpers.c(0), (vn) => dsl.helpers.c(1))
    ]

    let objective_names = []

    // iterate over optimization_objective_functions
    for(let i = 0; i < optimization_objective_functions.length; i++) {
      const optimization_objective_function = optimization_objective_functions[i]
      const name = 'min_objective' + i
      rosette_functions[name] = optimization_objective_function
      objective_names.push(name)
    }
    ...
    return {
      query: {
        ...
        "options": {
          "max_duplicates": 1,
          "check_acyclic": false,
          "minimization_criteria": objective_names
        },
