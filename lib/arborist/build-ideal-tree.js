...
module.exports = cls => class IdealTreeBuilder extends cls {
  ...
  async [_reachableNodesStep] (isContextNode, node, has_min_cve) {
    ...
    await Promise.all(dependencies.map(dep => {
      return this[_fetchPackuments](dep.constraintNpmSpec).then(async pack => {
        ...
        if(has_min_cve) {
          const envVar = process.env.CVE_ADVISORY_ENDPOINT
          const endpoint = envVar != undefined ? envVar : 'advisory.federico.codes'
          const url = `https://${endpoint}/api/cached/npm/${pack.name}`
          const vulns = await fetch(url).then(res => {
  ...
  async [_performRosetteQuery] (query) {
    ...
    if(this[_rosetteOnlyExplore]) {
      ...
      const envVar = process.env.Z3_ABS_PATH
      const z3Path = envVar != undefined ? envVar : projdir + "/bin/z3"
      await exec(`${projdir}/bin/solver/bin/rosette-solver ${queryPath} ${answerPath}`, 
        {env: {"Z3_ADD_MODEL_OPTION": "1", "Z3_ABS_PATH": z3Path}});
