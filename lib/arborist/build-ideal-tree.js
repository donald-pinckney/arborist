...
module.exports = cls => class IdealTreeBuilder extends cls {
  ...
  async [_buildRosetteQuery] () {
    ...
    const requested_consistency = this[_rosetteConsistencyArg] // 'npm'
    ...
    let rosette_functions = {
      ...
      ...combination_fn_defs,
      consistency: CONSISTENCY_RELATIONS[requested_consistency]
    }
  ...
  async [_reachableNodesStep] (isContextNode, node) {
    ...
    await Promise.all(dependencies.map(dep => {
      return this[_fetchPackuments](dep.constraintNpmSpec).then(async pack => {
        ...
        const vulns = JSON.parse(await getContent(`https://advisory.federico.codes/api/vuln/npm/${pack.name}`))

        let version_badnesses = Object.fromEntries(version_list.map((v_str) => {
          let badness = 0;
          for (const vuln of vulns) {
            const validRange = vuln.range.replace(', ', ' ')
            if (semver.satisfies(v_str, validRange, { loose: true })) {
