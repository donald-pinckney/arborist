...
const { ROSETTE_FUNCTIONS, CONSTRAINT_PRIMITIVES } = require('./rosette-functions.js')
...
const _buildPrereleaseOrder = Symbol('buildPrereleaseOrder')
...
const _rosetteConstraintFromRange = Symbol('rosetteConstraintFromRange')
...
module.exports = cls => class IdealTreeBuilder extends cls {
  ...
  async [_buildRosetteQuery] () {
    ...
    const allPrereleaseOrders = Object.fromEntries(Object.entries(allPrereleases).map(([name, prereleases]) => {
      return [name, this[_buildPrereleaseOrder](prereleases)]
    }))

    const rosetteContextNode = contextNode.map(d => {
      return {'packageToDependOn': d.packageToDependOn, 'constraint': this[_rosetteConstraintFromRange](d.semverRange, allPrereleaseOrders[d.packageToDependOn])}
    })
      

    const rosetteRegistry = Object.entries(reachableRegistry).map(([name, versions_obj]) => {
      const prereleaseOrder = allPrereleaseOrders[name]

      const versions_data = Object.entries(versions_obj).flatMap(([versionString, data]) => {
        const jsonRosetteVersion = this[_parseVersionString](versionString, prereleaseOrder) 
        ...
        const rosetteDeps = deps.map(d => {
          const constraint = this[_rosetteConstraintFromRange](d.semverRange, prereleaseOrder)
          return {'packageToDependOn': d.packageToDependOn, 'constraint': constraint}
        })
        return [{'version': jsonRosetteVersion, 'dependencies': rosetteDeps}]
      })
  ...
  [_buildPrereleaseOrder] (prereleases) {
  ...
  [_parseVersionString] (versionString, prereleaseOrder) {
  ...
  [_rosetteConstraintFromRange] (range, prereleaseOrder) {
    if(!range) {
  ...
  async [_reachableNodesStep] (isContextNode, node) {
    ...
    await Promise.all(dependencies.map(dep => {
      return this[_fetchPackuments](dep.constraintNpmSpec).then(pack => {
        ...
        const tmpPrereleaseOrder = this[_buildPrereleaseOrder](versionPrereleases.concat(depPrereleases))

        const constraint = this[_rosetteConstraintFromRange](dep.semverRange, tmpPrereleaseOrder)
        ...
        Object.entries(pack.versions).map(vers_pkg => {
          const jsonRosetteVersion = this[_parseVersionString](vers_pkg[0], tmpPrereleaseOrder)
          if(!jsonRosetteVersion) {
            return
          }
          ...
          if(this[_markNodeSeen](newNode)) {
            if(true || constraintFn(tmpRosetteVersion)) {
