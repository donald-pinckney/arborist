...
const { ROSETTE_FUNCTIONS_JSON } = require('./rosette-functions.js')
...
module.exports = cls => class IdealTreeBuilder extends cls {
  ...
  async [_buildRosetteQuery] () {
    ...
    return {
      ...
      "functions": ROSETTE_FUNCTIONS_JSON
    }
  }
  ...
  [_rosetteConstraintFromSpec] (spec) {
    const maybeWildcardBug = this[_tryParseWildcardBug](spec.fetchSpec)
    const maybeWildcardMinor = this[_tryParseWildcardMinor](spec.fetchSpec)
    ...
    if(spec.fetchSpec == 'latest' || spec.fetchSpec == '*') {
    ...
    } else if(spec.type == 'range' && spec.fetchSpec.startsWith('^')) {
      try {
        return {'caret': this[_parseVersionString](spec.fetchSpec.substring(1))}
      } catch {
        // If the version string is invalid, we'll just send an unsolvable constraint to Rosette
        return {'unsolveable': null}
      }
      ...
      try {
        return {'tilde': this[_parseVersionString](spec.fetchSpec.substring(1))}
      } catch {
        // If the version string is invalid, we'll just send an unsolvable constraint to Rosette
        return {'unsolveable': null}
      }
    } else if(spec.type == 'range' && spec.fetchSpec.startsWith('>=')) {
      const versionPart = spec.fetchSpec.substring(2).trim()
    ...
    } else if(spec.type == 'range' && maybeWildcardBug) {
      return maybeWildcardBug
    } else if(spec.type == 'range' && maybeWildcardMinor) {
  ...
  async [_buildReachableRegistry] (rootNode) {
    ...
    await this[_reachableNodesStep](true)
  }
  
  async [_reachableNodesStep] (isContextNode=false) {
    if(this[_reachableNodesQueue].length == 0) {
      return
    }
    ...
    if(alreadySeen) {
      console.log("Already saw: " + node.name + "@" + node.version)
