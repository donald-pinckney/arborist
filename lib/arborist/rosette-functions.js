...
const matchAssumePrereleaseOk = FunctionDef(1, [
  // matchAssumePrereleaseOk (lhs && rhs) :=
  //   let lhs_match = matchAssumePrereleaseOk lhs in
  //   let rhs_match = matchAssumePrereleaseOk rhs in
  //   λ v* → lhs_match v* && rhs_match v*
  ...
  new FunctionRule(
    ...
    new LambdaExpr('v*',
      new LetExpr('v*.major', op('vector-ref', theVar('v*'), 0), 
      new LetExpr('v*.minor', op('vector-ref', theVar('v*'), 1), 
      new LetExpr('v*.bug', op('vector-ref', theVar('v*'), 2), 
      new LetExpr('v*.prerelease', op('vector-ref', theVar('v*'), 3), 
...
// constraintInterpretation : Constraint -> Symb Version -> Boolean
